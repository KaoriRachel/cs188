...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC], variableDomainsDict[GHOST0], variableDomainsDict[GHOST1] = [], [], []
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            variableDomainsDict[PAC].append((i, j))
            variableDomainsDict[GHOST0].append((i, j))
            variableDomainsDict[GHOST1].append((i, j))
    variableDomainsDict[OBS0], variableDomainsDict[OBS1] = [], []
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # join and marginalize all factors in eliminationOrder
